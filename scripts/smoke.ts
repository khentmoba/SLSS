...
async function main(){
  ...
  for(const t of ['TCT_OCT','TAX_DECLARATION','VALID_ID'] as const){
    await prisma.document.create({data:{ projectId: proj.id, propertyId: prop.id, type: t as any, requirement:'REQUIRED', state:'MISSING' }});
  }
